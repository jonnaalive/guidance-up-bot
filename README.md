...
- 기업별 메시지에 Yahoo Finance의 시가총액을 억·조 단위, 원 통화, UTC 조회 시각과 함께 표시한다.
- 조회 시점 값이며 공시 시점의 과거 시총이나 환율 환산액이 아니다.
- 종목별 조회는 20초 제한, 동일 실행 내 캐시 적용. 실패·누락·종목 불일치는 '조회 불가'로 표시하고 알림을 계속한다.
- 일일보고는 조회 시작 예산 60초 후 추가 조회를 생략한다. 새 사건 없는 실행·투표 전용 실행은 조회하지 않는다.
- 기존 알림과 발송 대기 본문은 재작성하거나 재발송하지 않는다.
